...
public class HydroPlatinumDevice : DeviceBase
{
    ...
    protected readonly bool _passive;
    ...
    protected virtual void Initialize()
    {
        ...
        // In passive mode, we just read the sensors, we don't set default power
        if (!_passive)
        {
            SetChannelPower(PUMP_CHANNEL, DEFAULT_SPEED_CHANNEL_POWER);
        }
        
        _speedSensors[PUMP_CHANNEL] = new SpeedSensor("Pump", PUMP_CHANNEL, state.PumpRpm, supportsControl: !_passive);
        ...
        for (var i = 0; i < _fanCount; i++)
        {
            ...
            _speedSensors[i] = new SpeedSensor($"Fan #{i + 1}", i, state.FanRpm[i], supportsControl: !_passive);
        }
    }
        ...
        bool RebootImpl()
        {
            ...
            using (_guardManager.AwaitExclusiveAccess())
            {
                try
                {
                    ...
                    try
                    {
                        if (!_passive) WriteDirect(_dataWriter.CreateHidPacket(rebootPacket));
                    }
                    catch
                    {
                        // ignore
