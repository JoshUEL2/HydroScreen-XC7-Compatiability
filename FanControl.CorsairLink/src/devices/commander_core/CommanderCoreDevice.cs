...
public sealed class CommanderCoreDevice : DeviceBase
{
    ...
    {
        if (_passive) return true; // Skip in passive mode
    ...
    private IReadOnlyCollection<SpeedSensor> GetSpeedSensors(EndpointResponse connectedSpeedsResponse, EndpointResponse speedsResponse)
    {
        ...
        foreach (var speedSensor in speedSensors)
        {
            ...
            if (!_firstChannelExt)
            {
                sensors.Add(new SpeedSensor($"Fan #{i + 1}", i, speedSensor.Rpm, supportsControl: !_passive));
            }
            else
            {
                sensors.Add(new SpeedSensor(i == PUMP_CHANNEL ? "Pump" : $"Fan #{i}", i, speedSensor.Rpm, supportsControl: !_passive));
            }
        }
